...
function getAction(route) {
   ...
   logger.info("route.action:" + route.action);
   ...
   if (!h && this.getTemplateSource().exists) {
      h = function() {
         render.call(this);
      }
   }
   if (!h) h = (
      (action != "notfound") ? this.getAction({action:"notfound", format:format, method:method}) : null
   )
   return h;
}
